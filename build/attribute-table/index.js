...
function Edit(_ref) {
  ...
  const findTermID = (metaSuffix, termsArray) => {
    if (!termsArray.hasTerms || !meta['inventory_presser_' + metaSuffix]) {
      return false;
    }
  ...
  const updateTransmissionMetaValue = terms => newValue => {
    ...
    setMeta({ ...meta,
      "inventory_presser_transmission": term ? term.name : ''
    });
  };
  ...
  const updateDriveTypeMetaValue = terms => newValue => {
    ...
    const term = terms.hasTerms && terms.terms.find(term => term.id == newValue && 0 != term.id);
    setMeta({ ...meta,
      "inventory_presser_drive_type": term ? term.name : ''
    });
  };
  ...
  const updateFuelMetaValue = terms => newValue => {
    ...
    const term = terms.hasTerms && terms.terms.find(term => term.id == newValue && 0 != term.id);
    setMeta({ ...meta,
      "inventory_presser_fuel": term ? term.name : ''
    });
  };
  ...
  const updateCylindersMetaValue = terms => newValue => {
    ...
    const term = terms.hasTerms && terms.terms.find(term => term.id == newValue && 0 != term.id);
    setMeta({ ...meta,
      "inventory_presser_cylinders": term ? term.name : ''
    });
  };
  ...
  const updateBodyStyleMetaValue = terms => newValue => {
    ...
    const term = terms.hasTerms && terms.terms.find(term => term.id == newValue && 0 != term.id);
    setMeta({ ...meta,
      "inventory_presser_body_style": term ? term.name : ''
    });
  };
  ...
  const updateTypeMetaValue = terms => newValue => {
    ...
    const term = terms.hasTerms && terms.terms.find(term => term.id == newValue && 0 != term.id);
    setMeta({ ...meta,
      "inventory_presser_type": term ? term.name : ''
    });
  };
  ...
  const updateOdometerMetaValue = newValue => {
    //allow only digits
    setMeta({ ...meta,
      inventory_presser_odometer: newValue.replace(/[^0-9]+/g, '')
    });
  }; //Color
  ...
  const updateColorMetaValue = newValue => {
    setMeta({ ...meta,
      inventory_presser_color: newValue
    });
  }; //Interior Color
  ...
  const updateInteriorColorMetaValue = newValue => {
    setMeta({ ...meta,
      inventory_presser_interior_color: newValue
    });
  }; //Stock number
  ...
  const updateStockNumberMetaValue = newValue => {
    setMeta({ ...meta,
      inventory_presser_stock_number: newValue
    });
  }; //VIN
  ...
  const updateVinMetaValue = newValue => {
    setMeta({ ...meta,
      inventory_presser_vin: newValue
    });
  };
...
(0,_wordpress_blocks__WEBPACK_IMPORTED_MODULE_0__.registerBlockType)('create-block/invp-attribute-table', {
  /**
   * @see ./edit.js
   */
